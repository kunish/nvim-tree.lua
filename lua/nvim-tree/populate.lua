...
local function dir_new(cwd, name, status, parent_ignored)
	...
	return {
		...
		entries = {},
		git_status = parent_ignored and "!!"
			or (status.dirs and status.dirs[absolute_path])
			or (status.files and status.files[absolute_path]),
	}
end
...
local function should_ignore_git(path, status)
	return M.config.filter_ignored and (M.config.filter_git_ignored and status and status[path] == "!!")
end
	...
	local all = {
		{
			entries = dirs,
			fn = dir_new,
			check = function(_, abs)
				return luv.fs_access(abs, "R")
			end,
		},
		{
			entries = links,
...
function M.populate(entries, cwd, parent_node, status)
	...
	if parent_node and vim.g.nvim_tree_group_empty == 1 then
		if should_group(cwd, dirs, files, links) then
			local child_node
			if dirs[1] then
				child_node = dir_new(cwd, dirs[1], status, parent_node_ignored)
			end
			if links[1] then
				child_node = link_new(cwd, links[1], status, parent_node_ignored)
			end
